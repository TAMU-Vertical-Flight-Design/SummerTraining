...
cruise_conditions = asb.OperatingPoint(
    atmosphere=asb.Atmosphere(altitude=200), # 200 m altitude
    velocity=50, # 50 m/s airspeed
    alpha=7, # 7 deg angle of attack
)
...
span = optimization_environment.variable(init_guess=13, lower_bound=15, upper_bound=25) # m
root_chord = optimization_environment.variable(init_guess=1.2, lower_bound=0.3, upper_bound=3.0) # m
tip_chord = optimization_environment.variable(init_guess=.75, lower_bound=0.1, upper_bound=2) # m
...
# first airfoil
airfoil = asb.Airfoil("naca4412") # define the airfoil shape
...
vlm = asb.VortexLatticeMethod(
    ...
    spanwise_resolution=18, # panels spanwise per half-wing
...
optimization_environment.subject_to([ # .subject_to() sets the constraint
    taper_ratio >= 0.4, # minimum taper ratio
    taper_ratio <= 0.6 # maximum taper ratio
])
